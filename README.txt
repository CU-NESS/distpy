distpy is a clean and simple Python package meant to store analytical distributions efficiently and effectively. It has three submodules, distpy.transform, distpy.distribution, and distpy.jumping. In the main module (distpy.distribution) are implemented the distributions defined below (all are subclasses of base Distribution class). The distpy.transform module contains multiple transformations in the form of Transform objects which allow for distributions to be defined in transformed parameter spaces. The distpy.jumping module defines and subclasses the JumpingDistribution class which is very similar to the Distribution class except that JumpingDistribution objects require a source point to draw a destination point. This is particularly useful in defining Metropolis-Hastings MCMC samplers, one of which is contained in the distpy.jumping.MetropolisHastingsSampler (which is a subclass of emcee's Sampler object) class. Most of the defined JumpingDistributions are similar to the distributions below. distpy is compatible with Python 2.7+ and Python 3.5+. The only dependencies are numpy, scipy, matplotlib, and h5py (optional).
