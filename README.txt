The included univariate distributions (and their initializations, equals signs
...
(2) ParallelepipedDistribution(center, face_directions, distances,
                                                                norm_dirs=True)
...
(3) LinkedDistribution(shared_distribution, numparams)
    (pdf) --- f(x1, x2, x3, ...) =
                            shared_distribution(x1) * prod_{i=2}^N delta(xi-x1)
    (support) --- x1=x2=x3=...=xN where all are in support of
                                  shared_distribution
...
(4) SequentialDistribution(shared_distribution, numparams)
    (pdf) --- f(x1, x2, x3, ...) = prod_{i=1}^N shared_distribution(xi)
                                                         * is_sorted(x1,x2,...)
...
(6) EllipticalDistribution(mean, cov)
