"""
File: examples/distribution/bernoulli_distribution.py
Author: Keith Tauscher
Date: Oct 15 2019

Description: Example of using the BernoulliDistribution class.
"""
import os, time
import numpy as np
import matplotlib.pyplot as pl
from distpy import BernoulliDistribution

sample_size = int(1e5)

distribution = BernoulliDistribution(0.3)
hdf5_file_name = 'TEST_DELETE_THIS.hdf5'
distribution.save(hdf5_file_name)
try:
    assert distribution == BernoulliDistribution.load(hdf5_file_name)
except:
...
print(('It took {0:.5f} s to draw {1} points from a Bernoulli ' +\
...
ax.hist(sample, bins=np.arange(-0.5, 2, 1), histtype='step', color='b',\
...
ax.set_title('Bernoulli distribution test', size='xx-large')
