"""
File: examples/jumping/univariate_gaussian_jumping_distribution.py
Author: Keith Tauscher
Date: 22 Dec 2017

Description: Example of using the GaussianJumpingDistribution class to
             represent 1D Gaussian random variates.
"""
import os, time
import numpy as np
import matplotlib.pyplot as pl
from distpy import GaussianJumpingDistribution,\
...
umean2 = 15.
uvar = 2.5
distribution = GaussianJumpingDistribution(uvar)
...
ylim = pl.ylim()
pl.ylim(ylim)
pl.title(('Univariate Gaussian distributions variance={!s}').format(uvar),\
    size='xx-large')
